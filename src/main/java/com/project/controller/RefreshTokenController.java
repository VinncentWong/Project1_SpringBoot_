...
public class RefreshTokenController {
    ...
    public AppResponse getRefreshToken(@RequestBody JwtToken token, @RequestBody JwtToken bodyRefreshToken) throws Exception{
        String role = "";
        try{
            Claims exp = Jwts
                            .parser()
            ...
            for(Map.Entry<String, Object> pair: exp.entrySet()){
                if(pair.getKey().equals("exp")){
                    if(new Date(System.currentTimeMillis()).getTime() < Long.parseLong(pair.getValue().toString())){
                        ...
                        exp.put("key", new Date(new Date().getTime() + 1000000000L));
                    }
                }
            ...
            if(refreshToken.equals(bodyRefreshToken)){
